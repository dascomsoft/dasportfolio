...
const projectBox = [
...

      {
        ...
        role: "As a Full-Stack Developer and Frontend Engineer, I designed and built Boch237, a complete tutoring marketplace connecting parents/students with qualified tutors across Cameroon. I developed the entire architecture: Next.js frontend with TypeScript and TailwindCSS, Node.js/Express backend, MongoDB Atlas database, and real-time chat with Socket.IO. I implemented user authentication (JWT), admin dashboard, announcement system, profile management, message deletion/editing, and push notifications. I also deployed the frontend on Vercel and the backend on Render, ensuring a fully functional production environment. This project demonstrates my ability to build complex, real-world applications from scratch, manage both frontend and backend, and deliver a professional product ready for users.",
        problem: "In Cameroon, finding a qualified tutor is a real struggle. Parents and students rely on WhatsApp groups, Facebook, or word-of-mouth — with no transparency, no quality control, and no traceability. Good matches often happen by chance, and serious tutors struggle to get visibility. This leads to wasted time, uncertainty, and missed opportunities for both parents and tutors.",
        solution: "Boch237 is a centralized platform that solves these problems by offering: Advanced search by province, city, class, and subject,  Verified tutor profiles with ratings (coming soon),  Real-time secure chat (no phone number sharing),  Announcement system for parents (validated by admin), Complete admin dashboard for moderation,  Message deletion and editing (like WhatsApp),  Real-time notifications with unread badges,  Profile editing and password change. The platform is 100% mobile-first (85% of Cameroonian users are on mobile) and fully responsive. Built with Next.js, Node.js, Socket.IO, MongoDB Atlas, and deployed on Vercel + Render. Boch237 simplifies access to quality education in Cameroon, making the connection between families and tutors fast, transparent, and reliable. ",
        tech: "Next.js TypeScript TailwindCSS Node.js Express Socket.IO MongoDB Atlas JWT Vercel Render Lucide React Axios"
    },
